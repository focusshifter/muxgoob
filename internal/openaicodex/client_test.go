...
func TestClientCreateChatCompletionDoesNotFallBackWhenDisabled(t *testing.T) {
	attempt := 0
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		attempt++
		w.Header().Set("Content-Type", "text/event-stream")
	...
	codexHome := t.TempDir()
	writeAuthFile(t, codexHome, "test-access-token")
	fallback := &fallbackClientStub{resp: openai.ChatCompletionResponse{Choices: []openai.ChatCompletionChoice{{Message: openai.ChatCompletionMessage{Content: "must not be used"}}}}}
	client := NewClient(WithBaseURL(server.URL), WithCodexHome(codexHome), WithFallbackClient(fallback))
	resp, err := client.CreateChatCompletion(WithoutFallback(context.Background()), openai.ChatCompletionRequest{
		Model:    "openai/gpt-5.6-terra",
		Messages: []openai.ChatCompletionMessage{{Role: openai.ChatMessageRoleUser, Content: "draw a cat"}},
	})
	if err != nil {
		t.Fatalf("CreateChatCompletion error: %v", err)
	}
	if attempt != 2 || len(fallback.requests) != 0 {
		t.Fatalf("expected two Codex attempts and no fallback, attempts=%d fallback=%d", attempt, len(fallback.requests))
	}
	if len(resp.Choices) != 1 || resp.Choices[0].Message.Content != "" {
		t.Fatalf("expected empty Codex response, got %#v", resp)
	}
}

...
const imageParticipantFactBindingInstructions = "Treat every getUserFacts result item as a sealed record keyed by its query, user_id, and name: facts belong only to that returned person. For each depicted person, read the explicit appearance array and copy every applicable element into the generateImage prompt as a mandatory visual constraint; do not try to rediscover appearance inside the general facts dossier. When appearance contains distinct real-world and fictional/persona variants, select the variant matching the exact name requested: an account handle uses its real-world depiction, while a fictional/persona depiction is used only when that persona is explicitly requested. Never mix variants or copy, transfer, or reuse one person's facts for another person. If appearance is empty, leave the person's appearance unspecified or use a neutral labeled avatar; do not borrow another participant's traits or infer gender or appearance from a name."
...
func imagePromptComposer(chatID *int64) (chattools.ChatCompletionCreator, string, bool) {
	...
	model := strings.TrimSpace(registry.GetImagePromptModel(chatID))
	if model == "" || strings.TrimSpace(registry.Config.OpenrouterApiKey) == "" {
		return nil, "", false
	}
...
func askChatGptWithMode(message *telebot.Message, memoryAdmin bool, mutationTracker *memoryMutationTracker) string {
	...
	imagePromptMode := "off"
	composerClient, composerModel, composerConfigured := imagePromptComposer(chatID)
	if isImageGenerationRequest && imageTool != nil {
		imagePromptMode = registry.GetImagePromptMode(chatID)
	}
	if imagePromptMode == "direct" && !composerConfigured {
		log.Printf("[reply] Image prompt composer is in direct mode but OpenRouter provider/model is not configured")
		return ""
	}
	...
	if err != nil {
		...
		if imagePromptMode != "fallback" || !composerConfigured {
			return ""
		}
	}

	if imagePromptMode == "fallback" && !imageTool.WasSent() {
		if !composerConfigured {

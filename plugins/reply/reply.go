...
const imageParticipantFactBindingInstructions = "Treat every getUserFacts result item as a sealed record keyed by its query, user_id, and name: facts belong only to that returned person. Account handles and chat aliases refer to the same person, not separate personas. For each depicted person, read the explicit appearance array and copy every element into the generateImage prompt as a mandatory visual constraint; do not try to rediscover appearance inside the general facts dossier. Never omit one of that person's supplied visual traits, and never copy, transfer, or reuse one person's facts for another person. If appearance is empty, leave the person's appearance unspecified or use a neutral labeled avatar; do not borrow another participant's traits or infer gender or appearance from a name."
